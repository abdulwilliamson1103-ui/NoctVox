...
async function run() {
  ...
  const { error: sessErr } = await db.from('aum_sessions').insert({
    ...
    secondary_ring: 'R-SR',
    extended_rings: [],
    echo_blend: { lead_echo: 'E-TU', flavor_echo: 'E-CR', warmth: 0.72, confidence: 0.83 },
    expression_mode: 'H4:T-HR:R-VU:E-TU',
    alignment_status: 'aligned',
    confidence_score: 0.85,
    torch_weights: { 'T-HR': 32, 'T-RO': 22 },
    ring_weights: { 'R-VU': 38, 'R-SR': 22 },
  ...
  const { error: memErr } = await db.from('aum_memories').insert({
    user_id: testUserId,
    house_id: 4,
    emotional_signature: 'T-HR',
    content: '[Home/T-HR] User wants to buy a house in Charlotte — exploring property market.',
    mass_contribution: 3,
